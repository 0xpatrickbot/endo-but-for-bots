...
test('mount file streamBase64 yields base64-encoded chunks', async t => {
  ...
  // streamBase64 returns an async-iterator-ref whose chunks are
  // base64-encoded strings.  Decoding each chunk and concatenating
  // the decoded bytes must reproduce the original file content,
  // matching the docstring contract that wraps makeReaderRef (which
  // pipes the raw reader through encodeBase64 per chunk).
  const iterator = makeRefIterator(await E(file).streamBase64());
  const chunks = [];
  for (;;) {
    // eslint-disable-next-line no-await-in-loop
    const result = await iterator.next();
    if (result.done) break;
    t.is(typeof result.value, 'string', 'each chunk is a base64 string');
  ...
  // @endo/base64 mirrors what reader-ref.js feeds into encodeBase64 on
  // the producing side, so this is the canonical round-trip the
  // streamBase64 contract promises.
