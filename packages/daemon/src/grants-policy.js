...
/** @import { EndoProvisionPersistence, EndoProvisionPolicy, EndoProvisionPowerSpec, EndoProvisionSpec, GitGrant, MountGrant, NormalizeEndoProvisionOptions, NormalizedGitGrant, NormalizedGitRemoteSpec, NormalizedMountGrant } from './grants-types.js' */
...
const POWER_FIELDS = harden(['from']);
 ...
 * @param {unknown} value
 * @param {string} name
 * @returns {EndoProvisionPowerSpec}
 */
const normalizePower = (value, name) => {
  assertBindingName(name, `Power name ${name}`);
  const power = requirePlainRecord(value, `powers.${name}`);
  assertKnownFields(power, POWER_FIELDS, `powers.${name}`);
  const from = requireStringArray(power.from, `powers.${name}.from`);
  if (from.length === 0 || !from.every(isPetName)) {
    throw makeError(
      X`powers.${q(name)}.from must be a non-empty host pet-name path`,
    );
  }
  return harden({ from: harden([...from]) });
};
...
const normalizePolicy = async (spec, cwd, pinnedGitRoots = undefined) => {
  ...
  const powersRecord =
    root.powers === undefined
      ? undefined
      : requirePlainRecord(root.powers, 'EndoProvisionSpec.powers');
  const powers = /** @type {Record<string, EndoProvisionPowerSpec>} */ (
    Object.fromEntries(
      Object.keys(powersRecord ?? {})
        .sort()
        .map(name => [
          name,
          normalizePower(
            /** @type {Record<string, unknown>} */ (powersRecord)[name],
            name,
          ),
        ]),
    )
...
export const validateEndoProvisionPersistence = async value => {
  ...
  assertKnownFields(
    policyRecord,
    harden(['mounts', 'gits', 'gitRemotes', 'powers']),
    'Endo provision persistence.policy',
  );
  ...
  const persistedPowers =
    policyRecord.powers === undefined
      ? undefined
      : requirePlainRecord(
          policyRecord.powers,
          'Endo provision persistence.policy.powers',
        );
  ...
  const reconstructedSpec = harden({
    ...
    ...(persistedPowers === undefined ? {} : { powers: persistedPowers }),
  });
