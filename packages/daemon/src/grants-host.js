...
/** @import { EndoGuest, EndoHost, EndoMount } from './types.js' */
/** @import { GitRemote, GitRemoteController } from '@endo/exo-git' */
/** @import { EndoProvisionForkOptions, EndoProvisionPersistence } from './grants-types.js' */
 ...
 */
const resolvePowerIdentifiers = async (host, persistence) => {
  await null;
  const identifiers = new Map();
  for (const [name, power] of Object.entries(persistence.policy.powers ?? {})) {
    // eslint-disable-next-line no-await-in-loop
    const id = await E(host).identify(...power.from);
    if (typeof id !== 'string') {
      throw makeError(
 ...
 */
const assertRetainedPowerAliases = async (host, persistence) => {
  await null;
  const controllerPath = persistence.guestHandlePath.slice(0, -1);
  for (const name of Object.keys(persistence.policy.powers ?? {})) {
 ...
 * @returns {Promise<Map<string, string>>}
 */
const resolveRetainedPowerIdentifiers = async (host, persistence) => {
  await assertRetainedPowerAliases(host, persistence);
  const controllerPath = persistence.guestHandlePath.slice(0, -1);
  const identifiers = new Map();
  for (const name of Object.keys(persistence.policy.powers ?? {})) {
    const alias = harden([...controllerPath, 'powers', name]);
    ...
    if (typeof id !== 'string') {
      throw makeError(X`Retained power ${q(name)} has no formula identifier`);
    }
    // eslint-disable-next-line no-await-in-loop
    await E(host).lookupById(id);
    identifiers.set(name, id);
 ...
 * @returns {Promise<Map<string, string>>}
 */
const resolveForkPowerIdentifiers = async (host, parent, child) => {
  await null;
  ...
  return resolveRetainedPowerIdentifiers(host, normalizedParent);
};
...
const realizeProvisionResources = async (
  ...
  await ensureNameDirectory(host, [persistence.guestHandlePath[0]]);
  ...
  const powerEntries = Object.entries(persistence.policy.powers ?? {});
  if (powerEntries.length > 0) {
    const powersPath = harden([...controllerPath, 'powers']);
    await ensureNameDirectory(host, powersPath);
  }
  for (const [name] of powerEntries) {
    const powerAlias = harden([...controllerPath, 'powers', name]);
    if (powerIdentifiersToInstall !== undefined) {
      // eslint-disable-next-line no-await-in-loop
      if (await hasNamePath(host, powerAlias)) {
        throw makeError(
      ...
      // eslint-disable-next-line no-await-in-loop
      await E(host).storeIdentifier(powerAlias, id);
      // eslint-disable-next-line no-await-in-loop
    } else if (!(await hasNamePath(host, powerAlias))) {
      throw makeError(
        X`Retained power ${q(name)} is missing; refusing to re-resolve its host source`,
      );
    }
...
export const realizeEndoProvisionOnHost = async (
  ...
  if (options.forkFrom !== undefined) {
    ...
    await assertRetainedPowerAliases(host, normalizedPersistence);
  } else {
    await null;
    powerIdentifiersToInstall = await resolvePowerIdentifiers(
      host,
      normalizedPersistence,
    );
  }
