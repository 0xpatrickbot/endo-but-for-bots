 ...
 *   reachable only over the UDS bootstrap (this method) and via
 ...
 *   this only when both `udsBootstrap` and `adminDaemon` toggles
...
export const makeGatewayBootstrap = ({
  ...
  const registerInternal = (publicKey, nonce, signature, extras) => {
    ...
    if (extras.relayTarget !== undefined) {
      ...
      // `listRegistrations` reports the relay target. The policy
