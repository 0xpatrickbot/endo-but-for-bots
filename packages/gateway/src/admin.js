 ...
 * to the gateway's bootstrap socket holds the administrator's
 ...
 *   2. Over the local UDS bootstrap, via
 *      `E(bootstrap).getAdmin()`. The filesystem permissions on the
 *      socket gate who-may-call.
 *
 ...
 * the UDS bootstrap. The HTTP / WS surface (which lands in later
 * phases) does not expose `GatewayAdmin`; the gateway's
 * `getBootstrap` throws when `udsBootstrap` is disabled, and
 * `getAdmin` throws when `adminDaemon` is disabled or when
 * `udsBootstrap` is disabled (the admin daemon depends on the UDS
 ...
 * @property {() => ReadonlyArray<RegistrationSummary>} listRegistrations
  ...
  const exo = makeExo(
    ...
    /** @type {any} */ ({
      async listRegistrations() {
        return backplane.listRegistrations();
      },
      ...
      async getCounters() {
        const registrations = backplane.listRegistrations();
        let totalWeblets = 0;
