...
/** @import { FamiliarPublisher } from './src/familiar-publish.js' */
 ...
 * @property {FamiliarPublisher} [familiarPublish] Required when
 *   `familiarBundled` is enabled (Feature 5). The publisher writes
 *   the gateway's resolved bind address (the host:port pair
 *   `getBindAddress` returns after `start()`, which differs from
 *   the configured value when the configured port is `0`) to a
 *   caller-configurable file so the Familiar Electron shell's
 *   `localhttp://` protocol handler can proxy to the right
 *   address. `start()` invokes `publish` post-bind; `stop()`
 *   invokes `cleanup` so a quitting Familiar does not leave a
 *   stale port behind. A `familiarBundled=true` toggle with no
 *   publisher is a startup error (fail-closed); supply the
 *   package's own Node-backed publisher via
 *   `makeFamiliarPublisher({ io: makeNodeFamiliarPublishPowers(),
 *   publishPath })`, or an embedder-shaped equivalent for Endor /
 *   browser hosts.
 */
...
export const makeGateway = ({ powers = {}, config: configIn = {} } = {}) => {
  ...
  // `udsBootstrap`, `adminDaemon`, `gitHttp`, `captpRelay` all
