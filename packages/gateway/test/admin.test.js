...
const standDirect = () => {
  ...
  const admin = makeGatewayAdmin({
    backplane: {
      listRegistrations: handle.listRegistrations,
...
test('Gateway getAdmin throws when udsBootstrap is disabled', async t => {
  // Regression: admin depends on the UDS bootstrap for its access
  // channel; without UDS, there is no path for the administrator
  // to reach the facet, and the config validator already rejects
  // the combination. The in-process accessor mirrors the
  // contract by refusing to hand the facet out.
  await t.throws(
    () =>
      makeGateway({
...
test('Bootstrap getAdmin returns the admin exo over the UDS-mediated surface', async t => {
  // The "UDS-mediated" qualifier in the test name is the surface
  // contract: a process that holds a `GatewayBootstrap` got there
  // through the UDS bootstrap socket (or through the in-process
...
test('Bootstrap getAdmin throws when admin is disabled', async t => {
  // Configure with udsBootstrap on but adminDaemon off; the
