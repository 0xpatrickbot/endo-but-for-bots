...
(Feature 7), phase 2's UDS bootstrap registrar (Feature 4), and the
phase-1 skeleton's package shape. Phase 5 adds the data model and
admission surface for Feature 6 (public CapTP relay): per-
registration `relayPolicy` (`'closed'` by default, `'open'` opt-in)
plus a caller-public-key allowlist. The handler consults the policy
