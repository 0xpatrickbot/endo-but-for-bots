...
This is the **phase-3 slice**, building on phase 2's UDS bootstrap
...
(`gateway.getAdmin()`) and the UDS bootstrap (`bootstrap.getAdmin()`);
it is **never** exposed on the public HTTP / WS surface, keeping
admin authority off the network per the design.
