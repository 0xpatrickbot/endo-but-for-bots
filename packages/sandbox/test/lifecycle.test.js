...
/**
 * @typedef {(ms: number) => { promise: Promise<void>, cancel: () => void }} TestOnlyDelay
 * @param {{ spawnGate?: Promise<unknown>, afterAdmission?: () => void, softRefuse?: boolean, brokenSignals?: boolean, lifecycle?: boolean, context?: any, testOnlyDelay?: TestOnlyDelay }} [options]
 */
const makeDriverFixture = (options = {}) => {
  ...
  const factoryInput = {
    ...
    ...(options.testOnlyDelay === undefined
      ? {}
      : { testOnlyDelay: options.testOnlyDelay }),
  };
  const factory = makeSandboxFactory(factoryInput);
