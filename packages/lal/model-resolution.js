 ...
 *
 * pi-ai expects a single "provider/modelId" string and reads provider
 * API keys from `process.env.<PROVIDER>_API_KEY`. lal's historical
 * configuration passes LAL_HOST + LAL_MODEL + LAL_AUTH_TOKEN. The
 * helpers in this module translate the legacy LAL_* variables into
 * the pi-ai shape so existing `.env.example` files continue to work.
 *
 ...
 * an OpenAI-compatible /v1/chat/completions endpoint, so we masquerade
 * as the "openai" provider with a custom baseUrl. Matches the shape
 * genie uses internally.
 *
 ...
 * the resolution genie's `makePiAgent` performs internally: known
 * providers go through `getModel(provider, modelId)`; the `ollama/`
 * prefix is treated specially (Ollama is not in pi-ai's built-in
 * registry and exposes an OpenAI-compatible /v1 endpoint).
 *
...
export async function resolveModel(modelString) {
  ...
  // pi-ai's KnownProvider overloads of getModel typically resolve the
  // modelId to `never` for the generic call site; we want the runtime
  // registry lookup here, which works for any string the caller passed.
...
/**
 * API-key resolver for Ollama models. Ollama itself does not require a
 * key, but pi-ai's openai-completions adaptor refuses requests without
 * one. Prefer `OLLAMA_API_KEY` (in case the operator has set one for a
 * remote Ollama), else fall back to a harmless sentinel that the
 * operator's setup commonly uses already.
 *
 * @returns {string}
 */
