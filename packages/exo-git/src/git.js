 ...
 * `makeGitRemote({ operations })` by the caller that built both — the
 * caller that never happened is the one place `GitRemote` construction can
 * no longer silently recover a backend for a foreign, non-daemon-minted
 * `git` value, which is the authority leak the removed `gitBackends`
 * side-table and `getGitBackend` export risked.
 *
 ...
 * one.
 *
 * @param {{ backend: GitBackend }} powers
 * @returns {GitOperations}
 */
export const makeGitOperations = ({ backend }) => {
  if (backend === undefined || typeof backend !== 'object') {
...
/**
 * @param {{ message?: string, entries?: readonly object[], paths?: string[], includeUntracked?: boolean }} options
 * @this {GitMethodThis}
 */
...
export const makeGitKit = powers =>
  /** @type {any} */ (makeGitKitInstance(powers));
harden(makeGitKit);
