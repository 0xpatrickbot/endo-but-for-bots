...
- `makeGitOperations({ backend })` — mints the host-private `GitOperations` capability (the backend authority) alongside a Git kit, from the same `powers`. Never guest-visible and not derivable from `reader` / `writer` / `rewriter`; composing host code that built both passes `operations` explicitly to `makeGitRemote`.
...
- Interface guards: `GitReaderInterface`, `GitWriterInterface`, `GitRewriterInterface` (one per kit facet, generated from the shared `GIT_METHOD_GUARDS` table in `src/interfaces.js`), `GitInterface` (compatibility export equivalent to `GitRewriterInterface`, used by `@endo/agent-tools`'s JSON-tool and code-mode prompt generation), `GitRemoteInterface`, `GitRemoteControllerInterface`, `GitCredentialControllerInterface`, `BasicCredentialInterface`, `BearerCredentialInterface`.
